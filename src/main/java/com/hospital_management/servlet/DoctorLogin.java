	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			if (doctor != null) {
				...
				byte[] decodedBytes = decoder.decode(doctor.getPassword());
				String decodedPassword = new String(decodedBytes);
				if (password.equals(decodedPassword)) {
					HttpSession session = req.getSession();
					session.setAttribute("username", username);
					session.setAttribute("name", doctor.getName().split(" ")[0]);
					...
					rd = req.getRequestDispatcher("doctor_login.jsp");
					rd.include(req, resp);
				}
			} else {
				req.setAttribute("usernameMsg", "Invalid username. Please try again..!");
			...
			req.setAttribute("message", "An error occurred while loging the doctor..!");
			rd = req.getRequestDispatcher("doctor_login.jsp");
			rd.include(req, resp);
			e.printStackTrace();
		}
